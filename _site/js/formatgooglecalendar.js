...
var formatGoogleCalendar = (function() {
    ...
    var transformationList = function(result, tagName, format) {
        ...
        var dateFormatted = getFormattedDate(dateStart, dateEnd, moreDaysEvent, isAllDayEvent, dayNames),
            ...
            output = '<' + tagName + ' class="clearfix col-md-6 col-lg-4"><a class="event-link" href="' + link + '" target="_blank">',
        ...
        for (i = 0; i < format.length; i++) {
            ...
            if (format[i] === '*summary*') {
                ...
                output = output.concat('<div class="date"><span class="binds"></span><span class="month">' + dateFormatted[0] + '</span><span class="day">' + dateFormatted[1] + '</div>');
    ...
    var formatDateSameDay = function(dateStart, dateEnd, moreDaysEvent, isAllDayEvent, dayNames) {
        ...
        return dayNameStart + getMonthName(dateStart[1]);
    };

    // ***********************************
    var formatDateSameDay1 = function(dateStart, dateEnd, moreDaysEvent, isAllDayEvent, dayNames) {
        var formattedTime = '',
            dayNameStart = '';
        ...
        //month day, year time-time
        return dayNameStart + dateStart[0];
    };
        ...
        if (dateStart[0] === dateEnd[0]) {
            if (dateStart[1] === dateEnd[1]) {
                if (dateStart[2] === dateEnd[2]) {
                    //month day, year
                    formattedDate.push(formatDateSameDay(dateStart, dateEnd, moreDaysEvent, isAllDayEvent, dayNames), formatDateSameDay1(dateStart, dateEnd, moreDaysEvent, isAllDayEvent, dayNames));
                } else {
                    //month day, year - month day, year
                    formattedDate.push(formatDateDifferentYear(dateStart, dateEnd, dayNames));
                }
            } else {
                if (dateStart[2] === dateEnd[2]) {
                    //month day - month day, year
    ...
    return {
        init: function (settingsOverride) {
            var settings = {
                ...
                itemsTagName: 'li',
                upcomingSelector: '#events',
                pastSelector: '#events-past',
                upcomingHeading: '<h2>Upcoming events</h2>',
                pastHeading: '<h2>Past events</h2>',
                format: ['*date*', '<br> ', '*summary*', ' &mdash; ', '*description*', ' in ', '*location*']
            };
